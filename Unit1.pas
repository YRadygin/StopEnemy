...
{ Проверка наличия врагов в локальной солнечной системе, отображаются в виде
 ...
 участок скриншота побитово переберается в поисках заданного цвета
  ...
  begin
    ...
    SystemDyrectory := GetSpecialPath(CSIDL_PROFILE); //получили системную директорию  CSIDL_PROFILE - папка пользователя
    ...
    bmp2.Width:=W;
    bmp2.Height:=H;
    //Копирует часть скриншота по координате верхнего левого угла и высоте и ширине в bmp2
    BitBlt(bmp2.Canvas.Handle, 0, 0, W, H, bmp.Canvas.Handle, StrToInt(form1.Edit1.Text), StrToInt(form1.Edit2.Text), SRCCOPY);
    bmp2.PixelFormat:=pf8bit;  // Переводим в 8ми битную картинку скопированный участок
    { Если отслеживание запущено то представляем нашу вырезанную картинку в виде массива
    и проходим его попиксельно сравнивая с заданным в форме цветом, если в картинке присутствует заданный текст включаем тревогу}
    ...
    //Для наглядности выводим вырезанную часть скриншота в форме
    form1.Image1.Picture.Bitmap.Assign(bmp2);
    //Высвобождаем переменные
    png.Free;
    bmp.Free;
    bmp2.Free;
    //Очищаем директорию со скриншотами
...
procedure TForm1.Button1Click(Sender: TObject);
   //Обработка кнопки тест координат
  var
  TestStart : boolean;
...
//   Заполняем список выбора открытыми окнами, очищаем папку со скриншотами
    var
    wnd: hwnd;
...
begin
  ...
  killDir(SystemDyrectory+'\Documents\EVE\capture\Screenshots\');
  ComboBox1.clear;
  wnd := GetWindow(handle, gw_hwndfirst); //Получаем количество открытых окон
  while wnd <> 0 do
    begin // Не показываем:
...
{ При нажатии клавиши F11, F12 над клиентом игры задаем желаемый прямоугольник
 для отслеживания(координаты встают в поля ввода) }
...
//Активация основного таймера(старт отслеживания)
...
//Таймер основной проверки, при его в заданное на форме время происходит снятие скриншота
// и опрос картинки на предмет заданных цветовых пикселей
...
//Таймер который запускается при включении тревоги и в случае если не остановить тревогу
//Через 25 секунд все окна игры закроются.
